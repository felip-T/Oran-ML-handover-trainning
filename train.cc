...
OranLmCustomHandover::GetTypeId(void)
{
    static TypeId tid = TypeId("ns3::OranLmCustomHandover")
                            ...
                            .AddAttribute("nEnb", "Number of eNBs", UintegerValue(4),
    ...
    : OranLm()
{
    ...
    m_model = CreateObject<EnvOran>(4);
        ...
        if (found)
        {
            ...
            if (!nodePositions.empty())
            {
                ...
                try
                {
                    ...
                    std::string query = "SELECT sinr FROM sinr WHERE ueid = " + std::to_string(ueId) +
                                          ...
                                          std::vector<OranLmCustomHandover::EnbInfo> enbInfos) const
{
    ...
    for (auto ueInfo : ueInfos)
    {
        ...
        Ptr<OpenGymBoxContainer<double>> observation = CreateObject<OpenGymBoxContainer<double>>();
        observation->AddValue(ueInfo.cellId);
        observation->AddValue(ueInfo.apploss);
        observation->AddValue(ueInfo.sinr);
        for (auto dist : dists)
        {
            observation->AddValue(dist);
        }
